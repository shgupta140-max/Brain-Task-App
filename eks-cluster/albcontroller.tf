...
data "tls_certificate" "eks" {
  url = aws_eks_cluster.mindtrack.identity[0].oidc[0].issuer
}
...
data "aws_iam_policy_document" "lbc_assume_role_policy" {
  statement {
    ...
    condition {
      test     = "StringEquals"
      variable = "${replace(aws_eks_cluster.mindtrack.identity[0].oidc[0].issuer, "https://", "")}:aud"
      values   = ["sts.amazonaws.com"]
    }

    condition {
      test     = "StringEquals"
      variable = "${replace(aws_eks_cluster.mindtrack.identity[0].oidc[0].issuer, "https://", "")}:sub"
...
resource "helm_release" "aws_load_balancer_controller" {
  ...
  values = [
    yamlencode({
      clusterName = aws_eks_cluster.mindtrack.name
      vpcId       = aws_vpc.mindtrack.id
      serviceAccount = {
        create = true
        name   = "aws-load-balancer-controller"
        annotations = {
